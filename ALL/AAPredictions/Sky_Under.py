...
def is_there_nan(df,n,row=None):
    ...
    if row!=None:
        ...
        for i in df.columns:
            #print(df.iloc[n][i])
            #if pd.isnull(df.iloc[n][i]):
                #print(i,'aaaaaa')
            #print(pd.isnull(df.iloc[n][i]))
            if i=='Attendance': #é específico mas é necessário
                lista+=[False]
            else:
