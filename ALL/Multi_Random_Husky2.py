...
print('O objetivo desta versão (2) é manter as colunas por árvores')
print('Ao contrário da versão 1 que fazia random das colunas em cada Node')
...
def partition(df,Q):
    ...
    #print(f'Partition in {round(timeP2-timeP,5)} seconds')
    return true,false
...
def best_node_MULTI(df,count,forest=False,auto=True,columns_percentage=50,ind=[]): #melhor pergunta de todas as colunas
        ...
        if count==0:
            ...
            while not(done):
                ...
                if len(var_values(df,abcd))==1:
                    ...
                    print('The only value:',df.iloc[0][abcd])
                    #print(df,'DF')
                    #print('--------------------------------')
            ...
            return best_question, best_gain, ind
        else:
            best_question, best_gain=multi_columns(df,ind)
            return best_question, best_gain, ind
...
def build_tree(df,limit=None,forest=False,auto=True,columns_percentage=50,count=0,ind=[]):
    ...
    if len(df)<50:
        return Leaf(df)
    # # # # # # # Fim do específico
    q,gain,ind=best_node_MULTI(df,count,forest,auto,columns_percentage,ind=ind)
    if gain == 0:
        return Leaf(df)
    t, f =partition(df, q)
    ...
    #    true_branch=t
    #else:
    true_branch=build_tree(t,limit=limit,forest=forest,auto=auto,columns_percentage=columns_percentage,count=count,ind=ind)
    #if len(f)<50:
    #    false_branch=f
    #else:
    false_branch=build_tree(f,limit=limit,forest=forest,auto=auto,columns_percentage=columns_percentage,count=count,ind=ind)
...
#def old_var_values(df,column): #dá nos uma lista dos valores de uma variável 
