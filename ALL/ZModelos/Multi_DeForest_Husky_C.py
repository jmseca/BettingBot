...
def Big_dix(df,forest,T_df,var=1,tree_list=list(range(50,101)),
    ...
    for TL in tree_list:
        ...
        for FL in forest_list:
            ...
            df_aux3=df_aux3.reset_index(drop=True)
            #return df_aux1,df_aux2,df_aux3
            df_All=pd.concat([df_aux1,df_aux2,df_aux3],axis=1).reset_index(drop=True)
...
def Optimize_Forest(Forest1,Test_df,T_df,x=50):
    #x diz nos qual o limite de Predicted_% que aceitamos para a Opt_Forest
    New_F = Forest()
    df2=remove_columns(Test_df)
    df2=df2.values
    df2=T(df2,T_df)
    ...
    df2 = np.append(df2,index,axis=1) #juntamos a coluna index (e tem de estar na posição -1)
    proba_list = tree_proba(df2,Forest1)
    gt = good_trees(Test_df,proba_list).sort_values('Predicted_%',ascending=False)
    gt2=gt.drop_duplicates(subset=['Predicted_%','Not_Predicted_%','Predicted_Coef','Not_Predicted_Coef'])
    # até agr é igual ao Filt_Forest
    # agr entra a parte nova 
    gt3 = gt2.loc[gt2['Predicted_%']>=x]
    treex = list(map(lambda x: int(x),list(gt3['Tree'])))
    for t in treex:
        tree2 = Forest1.get_tree(t)
        New_F.add_tree(tree2)
    return New_F
