...
print('Usa um novo coeficiente, em vez do gini')
print('Foca-se mais no desempenho dos 1s')
...
def info_gain(true,false,current_purity):
...
def best_column_question(df,column,perguntas=1): #de uma coluna devolve a melhor pergunta e o seu info_gain
    ...
    if string_column(df,column):
        ...
        for val in values:
            ...
            t,f =partition(df,q)
            #se a pergunta não divide os dados em dfs >20 fazemos SKIP
            if len(t) == 20 or len(f) == 20:
                continue
            gain = info_gain(t,f,current_purity)
            if gain >= best_gain:
...
def build_forest(df,lista,none_limit=False,my_limit=4,forest=True,bootstrap=False,Min=6,Max=8,perguntas=2,count=0):
    ...
    for m in range(len(lista_ind)):
        colunas=lista_ind[m]
        if m<5: #dps apago, é só para ver se funciona
        ...
        if bootstrap:
            ...
            new_tree= build_tree(the_df,colunas,limit=limit,forest=forest,count=count,perguntas=perguntas)
            F.add_tree(new_tree)
...
def Q_columns(Forest,df): #devolve a quantidade de vezes que uma coluna foi escolhida para pergunta
    #df pode ser o test ou o Train
    df2 = remove_columns(df)
    cc=[]
    for t in range(Forest.size()):
        t0 = Forest.get_tree(t)
        cc += Qtree(t0)
    cc2 = {str(x):0 for x in df2.columns}
    for i in cc:
        cc2[str(df2.columns[i])]+=1
    return cc2

def Qtree(tree): #perguntas por árvores 
    if isinstance(tree,Leaf):
        return []
    else:
        a=Qtree(tree.true_branch)
        b=Qtree(tree.false_branch)
        c=[tree.question.column]
        return a+b+c
...
def Big_dix(df,forest,T_df,var=1,tree_list=list(range(50,101)),
    ...
    df_score=pd.DataFrame({'Index':list(range(len(df))),'League':df['League'],'Date':df['Date'],
                                ...
                                'Tree Score %':list(map(lambda_bigout2,bigout2,[F_siize]*len(bigout2))),
                                'Won/NotWon':df['Won/NotWon'],
                                'ODDH_Aver.':df['ODDH_Aver.'],'ODDD_Aver.':df['ODDD_Aver.'],'ODDA_Aver.':df['ODDA_Aver.']})
...
def Filter_Forest(Forest1,Test_df,T_df):
    ...
    gt = good_trees(Test_df,proba_list).sort_values('Predicted_%',ascending=True)
