...
def Portal_ao_teams(df,portal,league):
    ...
    if same!=[]:
        same_sky = [i[0] for i in same]
        missing_sky = []
        for i in df_t:
            if i not in same_sky:
                missing_sky+=[i]
        if missing_sky!=[]:
            missing_sky_copy = missing_sky.copy()
            ...
            while (missing_sky!=[]):
                ...
                if ok!='passa' and (ok not in missing_sky) and (ok!='reset'):
                    print('Erro na palavra')
                    continue
                if ok=='passa':
                    t+=1
                ...
                if ok=='reset':
                    t=0
                    df_p=df_p_copy.copy()
                    missing_sky=missing_sky_copy.copy()
                    same=same_copy.copy()
                    continue
                else:
                    same+=[[ok,df_p[t]]]
                    del df_p[t]
                    del missing_sky[np.where(np.array(missing_sky)==ok)[0][0]]
                if t==len(df_p):
                    t=0
        op = open('C://Users//joaom//Documents//Projetos//PYTHON//Apostas//PortalTeams_'+league+'.pickle','wb')
        pickle.dump(same,op)
...
def PortalAdd_Sky2(portal,husky):
#divide o to_add, nos jogos mesmo para adicionar e os outros
    #print('portal')
    #print(list(set(list(np.unique(list(portal['HT'])))+list(np.unique(list(portal['AT']))))))
    #print(' ')
    #print('husky')
    #print(list(set(list(np.unique(list(husky['HT'])))+list(np.unique(list(husky['AT']))))))
    dixH = {}
    remove1=[] #para o not_add1
    remove2 = [] # para o not_add2
    for t in list(set(list(np.unique(list(husky['HT'])))+list(np.unique(list(husky['AT']))))):
        dixH[t] = len(list(np.where(np.logical_or(husky['AT']==t,husky['HT']==t))[0])) 

    dixG = dixH.copy() # para controlarmos o número de jogos que a equipa já jogou
    for i in range(len(portal)):
        ht,at = portal.iloc[i]['HT'],portal.iloc[i]['AT']
        dixG[ht]+=1
        dixG[at]+=1

        if dixG[ht]>=6 and dixG[at]>=6:
            if (dixG[ht]>(dixH[ht]+1)) or (dixG[at]>(dixH[at]+1)):
                remove2+=[i]
            else:
                pass
    ...
    df_add = portal.drop(remove,axis=0)
    #df_add1 para adicionar ao husky
    #portal, para adicionar no final
    df_notadd2 = portal.loc[remove2].sort_values(['Date','Time'],ascending=[True,True]).reset_index(drop=True)
    #df_notadd2, são os jogos que ainda não conseguimos prever pq pelo menos uma das equipas ainda tem um jogo por jogar
    return df_add.reset_index(drop=True),portal,df_notadd2,len(df_add)